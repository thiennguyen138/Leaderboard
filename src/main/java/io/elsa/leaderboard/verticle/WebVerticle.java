      ...
      .handler(ctx -> {
        ...
        leaderBoardService.getLeaderboardInRange(from, to, rs -> {
          if (rs.succeeded()) {
            ...
            response.write(gson.toJson(rs.result()));
            ctx.response().end();
          } else {
      ...
      .handler(ctx -> {
        ...
        leaderBoardService.getRankByUserId(userId, rs -> {
          if (rs.succeeded()) {
            ...
            response.setChunked(true);
            if (rs.result() != null) {
              leaderBoardService.getRelativeLeaderboard(rs.result() - bound, rs.result() + bound, relRs -> {
                if (relRs.succeeded() && relRs.result() != null) {
                  response.write(gson.toJson(relRs.result()));
                  ctx.response().end();
                } else {
                  ctx.response().setStatusCode(500).end("Internal Server Error");
                }
