...
public class RedisVerticle extends AbstractVerticle {
    ...
    public void start() {
        ...
        options.setConnectionString("redis://:Passw0rd@" + config().getString("redisHost") + ":6379/0");
        ...
        Redis.createClient(vertx, options)
                ...
                .onSuccess(conn -> {
                    ...
                    vertx.eventBus().consumer(VertXAddress.GET_RANKS, message -> {
                        ...
                        redisAPI.zrevrange(Arrays.asList(LEADERBOARD, body.getString("from"), body.getString("to")), rs -> {
                            if (rs.succeeded()) {
                                if (rs.result() != null) {
                    ...
                    vertx.eventBus().consumer(VertXAddress.GET_FROM_CACHE, message -> {
                        JsonObject body = (JsonObject) message.body();
                        redisAPI.hget(body.getString("key"), body.getString("field"), rs -> {
                            if (rs.succeeded()) {
                                message.reply(rs.result() == null ? null : rs.result().toString());
                            } else {
                                message.fail(500, ExceptionUtils.getStackTrace(rs.cause()));
                            }
                        });
                    ...
                    vertx.eventBus().consumer(VertXAddress.PUT_TO_CACHE, message -> {
                        JsonObject body = (JsonObject) message.body();
                        redisAPI.hset(Arrays.asList(body.getString("key"), body.getString("field"), body.getString("value")), rs -> {
                            if (rs.succeeded()) {
